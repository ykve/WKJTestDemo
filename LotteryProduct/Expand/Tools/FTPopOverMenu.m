...
#define KFTSCREEN_WIDTH               [[UIScreen mainScreen] bounds].size.width
#define KFTSCREEN_HEIGHT              [[UIScreen mainScreen] bounds].size.height
...
#define FTDefaultTextColor          [UIColor colorWithRed:0/255.f green:0/255.f blue:0/255.f alpha:1.f]
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier menuName:(NSString *)menuName iconImageName:(NSString *)iconImageName
{
    ...
    if (self) {
        ...
        _menuNameLabel.textColor = [UIColor colorWithHex:@"f74444"];
...
-(void)adjustPopOverMenu
{
    ...
    [self.backgroundView setFrame:CGRectMake(0, 0, KFTSCREEN_WIDTH, KFTSCREEN_HEIGHT)];
    ...
    if (senderRect.origin.y > KFTSCREEN_HEIGHT) {
        senderRect.origin.y = KFTSCREEN_HEIGHT;
    }
    ...
    if (senderRect.origin.y + senderRect.size.height/2  < KFTSCREEN_HEIGHT/2) {
    ...
    if (menuArrowPoint.x + FTDefaultMenuWidth/2 + FTDefaultMargin > KFTSCREEN_WIDTH) {
        menuArrowPoint.x = MIN(menuArrowPoint.x - (KFTSCREEN_WIDTH - FTDefaultMenuWidth - FTDefaultMargin), menuArrowPoint.x);
        menuX = KFTSCREEN_WIDTH - FTDefaultMenuWidth - FTDefaultMargin;
    ...
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {
        ...
        if (menuRect.origin.y + menuRect.size.height > KFTSCREEN_HEIGHT) {
            menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height), FTDefaultMenuWidth, KFTSCREEN_HEIGHT - menuRect.origin.y - FTDefaultMargin);
