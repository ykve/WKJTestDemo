...
#define kDefaultFireIntervalHighUse  1
...
- (void) updateLabel:(NSTimer *) timer {
     ...
     startCountDate = [NSDate dateWithTimeIntervalSince1970:self.startTimeInterval];
    
    NSTimeInterval timeDieff = [[[NSDate alloc]init]timeIntervalSinceDate:startCountDate];
    NSDate * timeToShow = [NSDate dateWithTimeIntervalSince1970:self.startTimeInterval];
...
//                pausedTime = nil;
   ...
   NSArray *dateArray = [strDate componentsSeparatedByString:@"天"];
    //daniel
    if ([dateArray.firstObject intValue] > 0 && dateArray.count > 1) {
        
        NSInteger dayNum = [dateArray.firstObject intValue] ;
        if (dayNum >= 1) {
            dayNum -= 1;
        }
        NSString *dayStr = [NSString stringWithFormat:@"%02ld", (long)dayNum];
        NSString *newStr = [NSString stringWithFormat:@"%@天%@", dayStr, dateArray.lastObject];
        
        _timeLabel.text = newStr;
    }else{
