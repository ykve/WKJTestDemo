...
@interface YZDisplayViewController () 
{
    UIColor *_norColor;
    UIColor *_selColor;
}
/** <#Description#> */
@property (strong, nonatomic) UIView *titleShadowView;
/**
 *  下标宽度是否等于标题宽度
 */
...
- (UIView *)underLine
{
    if (_underLine == nil) {
        ...
        underLineView.backgroundColor = _underLineColor?_underLineColor:[UIColor colorWithRed:0.773 green:0.651 blue:0.427 alpha:1.000];
...
- (UIScrollView *)titleScrollView
{
    if (_titleScrollView == nil) {
        ...
        titleScrollView.backgroundColor = [[CPTThemeConfig shareManager] genDanHallTitleBackgroundColor];//[UIColor colorWithRed:0.180 green:0.184 blue:0.200 alpha:1.000];
...
{
    if (_titleShadowView == nil) {

        UIView *titleShadowView = [[UIView alloc] init];
        titleShadowView.backgroundColor = [UIColor clearColor];
        titleShadowView.layer.shadowColor = [[UIColor blackColor] colorWithAlphaComponent:0.5].CGColor;

        titleShadowView.layer.shadowOffset = CGSizeMake(0, 1);

        titleShadowView.layer.shadowOpacity = 1;

        titleShadowView.layer.shadowRadius = 1;

        titleShadowView.layer.cornerRadius = 10.0;

        titleShadowView.clipsToBounds = NO;

        titleShadowView.userInteractionEnabled = YES;

//        [titleShadowView addSubview:self.titleScrollView];
        [self.contentView insertSubview:titleShadowView atIndex:0];

        _titleShadowView = titleShadowView;
...
- (UIView *)titleBottomView {
    if (!_titleBottomView) {
        ...
        _titleBottomView.backgroundColor = [UIColor colorWithRed:240/255.0 green:240/255.0 blue:240/255.0 alpha:1];
        [self.contentView addSubview:_titleBottomView];
    }
...
- (void)viewDidLayoutSubviews
{
    ...
    self.underLine.backgroundColor = [[CPTThemeConfig shareManager] genDanHallTitleSelectColr];
    ...
    if (_isInitial == NO) {
        ...
        if (_isfullScreen) {
            ...
            // 整体contentView尺寸
            self.contentView.frame = CGRectMake(0, 0, YZScreenW, SCREEN_HEIGHT);
            
            // 顶部标题View尺寸
//            self.titleShadowView.frame = CGRectMake(9, 1, YZScreenW - 18, self.titleHeight);
            self.titleScrollView.frame = CGRectMake(0, 0, YZScreenW, self.titleHeight);
            
            self.titleBottomView.frame = CGRectMake(0, self.titleHeight, YZScreenW, 2);
            // 顶部内容View尺寸
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, titleY, YZScreenW, SCREEN_HEIGHT - titleY);
        }
        
        // 顶部标题View尺寸
//        self.titleShadowView.frame = CGRectMake(9, 1, YZScreenW - 18, self.titleHeight);
        self.titleScrollView.frame = CGRectMake(0, 0, YZScreenW, self.titleHeight);
        self.titleBottomView.frame = CGRectMake(0, self.titleHeight, YZScreenW, 0.25);
        
        // 顶部内容View尺寸
        CGFloat contentY = CGRectGetMaxY(self.titleScrollView.frame) + 0.25;
        CGFloat contentH = self.contentView.yz_height - contentY - NAV_HEIGHT;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    self.underLine.yz_width = 25;//underLineWidth;
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (YZDisplayTitleLabel *labelView in self.titleLabels) {
        ...
        labelView.font = YZTitleFont;
        labelView.backgroundColor = CLEAR;//[UIColor colorWithRed:0.180 green:0.184 blue:0.200 alpha:1.000];
...
//        }
    ...
    label.backgroundColor = CLEAR;//[UIColor colorWithRed:0.180 green:0.184 blue:0.200 alpha:1.000];
...
//        if (_isUnderLineEqualTitleWidth) {
        self.underLine.yz_width = 25;//titleBounds.size.width;
//        } else {
...
//        } else {
        self.underLine.yz_width = titleBounds.size.width;//label.yz_width;
//        }
        self.underLine.yz_centerX = label.yz_centerX;
    }];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat bottom = self.tabBarController == nil? SAFE_HEIGHT :49;
