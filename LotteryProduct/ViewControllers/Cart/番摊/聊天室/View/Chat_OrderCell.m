...
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
    UIImage *img = [UIImage imageNamed:@"bg-tuidan"];
...
- (void)setDataWithModel:(ChatMessageModel *)model{
    ...
    _levelImgView.image = [UIImage imageNamed:[NSString stringWithFormat:@"vip_chat_v%@",model.level]];
...
- (IBAction)followClick:(UIButton *)sender {
    if(_model.userId.integerValue == [Person person].uid.integerValue){
        ...
        @weakify(self);
        NSDictionary *dic = @{@"otherId":_model.orderModel.godId};
