...
@interface SQMenuShowView ()<UITableViewDelegate,UITableViewDataSource,showmissDelegate>
...
//    cell.missswitch.hidden = indexPath.row == 0 ? NO : YES;
    cell.missswitch.hidden = YES;
...
#pragma delegate 是否显示遗漏
-(void)showmiss:(BOOL)show {
    
    self.showmiss = show;
    
    if (self.showmissBlock) {
        
        self.showmissBlock(show);
    }
...
- (void)showView{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self.superview insertSubview:_overlayView atIndex:self.superview.subviews.count-1];
        [_overlayView addTarget:self action:@selector(dismissView) forControlEvents:UIControlEventTouchUpInside];
